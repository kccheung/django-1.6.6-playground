...
def create_choice(choice_text, poll_id):
    """
    Creates a choice
    """
    return Choice.objects.create(votes=0, choice_text=choice_text, poll_id=poll_id)
...
class PollIndexResultsTests(TestCase):
    def test_results_view_with_a_future_poll(self):
        """
        The results view of a poll with a pub_date in the future should return a 404 not found.
        """
        future_poll = create_poll(question='Future poll.', days=5)
    ...
    def test_results_view_with_a_past_poll(self):
        """
        The results view of a poll with a pub_date in the past should display the poll's question.
        """
        past_poll = create_poll(question='Past Poll.', days=-5)
